...
function updateWelcome(){
  // ── Launcher
  const swN=document.getElementById('sw-name');if(swN&&_userName)swN.textContent=_userName;
  const swN2=document.getElementById('sw-name2');if(swN2&&_userName)swN2.textContent=_userName;
  // ── Descarte inline
...
// ═══ SUITE LAUNCHER — estado e funções de navegação ══════════
let _activeModule=null;

const _SUITE_MODS={
  exp:{id:'__aa__',   name:'Auto Expedição'},
  dsc:{id:'__dsc__',  name:'Auto Descarte'},
  sol:{id:'__sol__',  name:'Solicitar Ativos'},
  caba:{id:'__caba__',name:'Confirma ABA'},
};

function showModule(mod){
  _activeModule=mod;
  Object.entries(_SUITE_MODS).forEach(([k,v])=>{
    const el=document.getElementById(v.id);
    if(el)el.classList.toggle('off',k!==mod);
  });
  const lnc=document.getElementById('__suite-welcome__');
  if(lnc){lnc.classList.add('sw-out');setTimeout(()=>{lnc.style.display='none';lnc.classList.remove('sw-out');},300);}
  document.querySelectorAll('.sw-nav-label[data-mod]').forEach(l=>l.classList.toggle('active',l.dataset.mod===mod));
}

function showLauncher(){
  _activeModule=null;
  Object.values(_SUITE_MODS).forEach(v=>{const el=document.getElementById(v.id);if(el)el.classList.add('off');});
  const lnc=document.getElementById('__suite-welcome__');
  if(lnc){lnc.style.display='flex';lnc.style.animation='sw-fade-in .3s ease forwards';}
  ...
  ov.innerHTML=
    '<div class="sw-card">'+
      ...
      '<nav class="sw-nav">'+
        ...
        '</label>'+
        '<label class="sw-nav-label" data-mod="sol" title="Solicitações">'+
      ...
      '</nav>'+
    '</div>';
  ov.querySelectorAll('.sw-opt[data-mod]').forEach(btn=>{
    btn.addEventListener('click',()=>showModule(btn.dataset.mod));
  });
  ov.querySelectorAll('.sw-nav-label[data-mod]').forEach(lbl=>{
    lbl.addEventListener('click',()=>showModule(lbl.dataset.mod));
  });
...
.aa-rtable tbody td{padding:6px 8px;border-bottom:1px solid var(--mg-b1);color:var(--mg-t1);}
...
.aa-final-modal{
  ...
  overflow:hidden;color:var(--mg-t1);
}
.aa-final-header{
...
/* ─── SUITE WELCOME LAUNCHER ────────────────────────────────── */
#__suite-welcome__{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:2147483647;background:rgba(10,12,18,0.90);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);animation:sw-fade-in .35s ease forwards;}
#__suite-welcome__.sw-out{animation:sw-fade-out .28s ease forwards;pointer-events:none;}
@keyframes sw-fade-in{from{opacity:0}to{opacity:1}}
@keyframes sw-fade-out{from{opacity:1}to{opacity:0}}
...
.dsc-welcome-av{width:32px;height:32px;border-radius:50%;background:var(--mg-blue);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#fff;flex-shrink:0;}
...
.dsc-hint:hover{border-color:var(--mg-blue);color:var(--mg-blue);background:rgba(0,120,230,0.08);}
...
.dsc-btn-run-wrap:hover .dsc-btn-run-inner{background:var(--mg-blue2);box-shadow:0 4px 18px rgba(0,120,230,0.38);transform:translateY(-1px);}
...
.dsc-progress-bar{height:100%;background:var(--mg-blue);border-radius:4px;transition:width .4s cubic-bezier(.4,0,.2,1);width:0%;}
...
/* MODAL FINAL */
.aa-final-modal{background:var(--mg-panel);border:1px solid var(--mg-b1);border-radius:18px;padding:0;max-width:460px;width:95%;box-shadow:0 24px 80px rgba(0,0,0,0.22);animation:modal-pop .3s cubic-bezier(.34,1.56,.64,1);overflow:hidden;color:var(--mg-t1);}
...
#exp-min{position:relative;}
#exp-min::before,#exp-min::after{content:'';position:absolute;left:50%;top:50%;border-radius:2px;transition:all .18s ease;}
#exp-min[data-state='open']::before{width:10px;height:2px;background:currentColor;transform:translate(-50%,-50%);}
#exp-min[data-state='open']::after{display:none;}
#exp-min[data-state='closed']::before{width:9px;height:9px;background:transparent;border:1.8px solid currentColor;border-radius:2px;transform:translate(-50%,-50%);}
#exp-min[data-state='closed']::after{display:none;}
...
async function startDsc(){
  ...
  const _dscTa=document.getElementById('dsc-ta');if(_dscTa)_dscTa.value='';
}
    ...
    m.innerHTML=
      ...
      '<div class="aa-final-body">'+
        ...
        '<div class="aa-final-btns">'+
          '<button class="aa-mb p" data-v="close" style="font-size:12px;padding:13px;letter-spacing:.5px;">Fechar</button>'+
        '</div>'+
      '</div>';
...
function buildPanel_dsc(){
  ...
  if(!document.body)return setTimeout(buildPanel_dsc,10);
  ...
  root.innerHTML=
    '<div class="dsc-header" id="dsc-drag-handle">'+
      ...
      '<div class="dsc-header-btns">'+
        '<button class="dsc-hbtn" id="dsc-switch" title="Trocar script" style="font-size:11px;">⊞</button>'+
  ...
  // SWITCH SCRIPT
  document.getElementById('dsc-switch').onclick=e=>{e.stopPropagation();showLauncher();};
  ...
  document.getElementById('dsc-lclr').onclick=e=>{e.stopPropagation();document.getElementById('dsc-lb').innerHTML='';_lc_dsc=0;document.getElementById('dsc-lc').textContent='0';};
...
function logDsc(msg,type){
  ...
  _lc_dsc++;const lc=document.getElementById('dsc-lc');if(lc)lc.textContent=_lc_dsc;
...
function buildPanel_sol(){
  ...
  if(!document.body)return setTimeout(buildPanel_sol,10);
  ...
  root.innerHTML=
    '<div class="sol-header" id="sol-drag-handle">'+
      ...
      '<div class="sol-header-btns">'+
        '<button class="sol-hbtn" id="sol-switch" title="Trocar script" style="font-size:11px;">⊞</button>'+
  ...
  // SWITCH SCRIPT
  document.getElementById('sol-switch').onclick=e=>{e.stopPropagation();showLauncher();};
  ...
  document.getElementById('sol-lclr').onclick=e=>{e.stopPropagation();document.getElementById('sol-lb').innerHTML='';_lc_sol=0;document.getElementById('sol-lc').textContent='0';};
  if(_userName)updateWelcome();
  uiTokenSol();
...
function logSol(msg,type){
  ...
  _lc_sol++;const lc=document.getElementById('sol-lc');if(lc)lc.textContent=_lc_sol;
  const t=new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
...
function buildPanel_exp(){
  ...
  if(!document.body)return setTimeout(buildPanel_exp,10);
    ...
    '<div class="sol-body">'+
      ...
      '<div class="sol-card">'+
        ...
        '<div class="sol-modo-desc" id="aa-modo-desc">'+
          ...
          '<br><span class="sol-gemco-example" style="margin-top:4px;opacity:.8;">550 - Monitor x1 &nbsp;→&nbsp; filial, produto, qtd</span>'+
        '</div>'+
      '</div>'+
      ...
      // TYPEWRITER
      '<div class="sol-typewriter-wrap" id="exp-typewriter">'+
        '<div class="sol-typewriter">'+
          '<div class="tw-slide"><i></i></div>'+
          '<div class="tw-paper"></div>'+
          '<div class="tw-keyboard"></div>'+
  ...
  const fmt='<br><span class="sol-gemco-example" style="margin-top:4px;opacity:.8;">550 - Monitor x1 &nbsp;→&nbsp; filial, produto, qtd</span>';
  const modoDescs={
    full:'Executa o fluxo <strong>completo</strong>: solicitação → separação → carga → conferência → NF-e → e-mails.<br><span class="sol-gemco-example">Recomendado para início do processo</span>'+fmt,
    sep:'Inicia a partir da <strong>separação</strong>, pulando solicitação.<br><span class="sol-gemco-example">Use quando a solicitação já foi feita</span>'+fmt,
    carga:'Inicia a partir da <strong>carga</strong>, pulando solicitação e separação.<br><span class="sol-gemco-example">Use quando separação já foi concluída</span>'+fmt
  };
  ...
  document.getElementById('aa-lclr').onclick=e=>{e.stopPropagation();document.getElementById('aa-lb').innerHTML='';_lc_exp=0;document.getElementById('aa-lc').textContent='0';};
...
function showWorkingExp(show){
  const tw=document.getElementById('exp-typewriter');
  if(tw)tw.classList.toggle('active',show);
}
function getMode(){
...
function logExp(msg,type='info'){
  ...
  _lc_exp++;
  const lc=document.getElementById('aa-lc');if(lc)lc.textContent=_lc_exp;
  const t=new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
  const d=document.createElement('div');d.className='sol-log-entry '+type;
  d.textContent=t+'  '+msg;lb.appendChild(d);lb.scrollTop=lb.scrollHeight;
  if(lb.children.length>200)lb.removeChild(lb.children[0]);
...
async function startExp(){
  ...
  if(!S_exp.stop){const _expTa=document.getElementById('aa-ta');if(_expTa)_expTa.value='';}
  document.getElementById('sol-stop-section').classList.remove('active');
  setProgExp(100);setTimeout(()=>setProgExp(null),600);
  setStExp(S_exp.stop?'Interrompido.':'Processo finalizado ✓',false);
  if(!S_exp.stop){logExp('Finalizado.','ok');await finalModal();}
}
...
async function finalModal(){
  ...
  return new Promise(res=>{
    ...
    const allFail=fails.length>0&&oks.length===0;
    const partial=fails.length>0&&oks.length>0;
    const hdrBg=allFail?'linear-gradient(135deg,#7f1d1d 0%,#991b1b 100%)':partial?'linear-gradient(135deg,#78350f 0%,#92400e 100%)':'linear-gradient(135deg,var(--mg-blue) 0%,#0055c8 100%)';
    const hdrEmoji=allFail?'😔':partial?'⚡':'🎉';
    const hdrTitle=allFail?'Ops, '+nome+'. Nada foi processado.':partial?'Quase lá, '+nome+'! ⚡':'Deu certo, '+nome+'! 🎉';
    const hdrSub=allFail?'Nenhuma filial foi encontrada ou processada. Verifique os dados e tente novamente.':(mm>0?mm+'m ':'')+ss+'s · '+( S_exp.modo||'');
    m.innerHTML=
      '<div class="aa-final-header" style="background:'+hdrBg+'">'+
        '<span class="aa-final-emoji">'+hdrEmoji+'</span>'+
        '<div class="aa-final-title">'+hdrTitle+'</div>'+
        '<div class="aa-final-subtitle">'+hdrSub+'</div>'+
      '</div>'+
      '<div class="aa-final-body">'+
        '<div class="aa-res-sum">'+
...
let logOpen=true;document.getElementById('caba-lh').onclick=()=>{logOpen=!logOpen;document.getElementById('caba-lb').style.display=logOpen?'':'none';};document.getElementById('caba-lclr').onclick=e=>{e.stopPropagation();document.getElementById('caba-lb').innerHTML='';_lc_caba=0;document.getElementById('caba-lc').textContent='0';};
...
function _initAll(){
  buildWelcome();
  setTimeout(()=>{ buildPanel_dsc(); },400);
  setTimeout(()=>{ buildPanel_sol(); },500);
  setTimeout(()=>{ buildPanel_exp(); },600);
  setTimeout(()=>{ buildPanel_caba(); },700);
}
